...
// 去除菜单url中的前缀: /developer
let trimMenuUrlPrefix = function(menus, prefix = '/developer'){
    menus.forEach((menu) => {
...
let getFirstMenu = function(menus){
    let firstRoute = '/developer/welcome';
    menus.forEach((menu) => {
...
export default new Vuex.Store({
    ...
    state: {
        ...
        systemName: '开发总后台',
        globalLoading: false,
        theme: deepCopy(defaultThemes['深灰']),
        user: null,
        menus: [], // 用户的菜单列表(树型结构)
        rules: [], // 用户的权限列表(列表结构)
        currentMenu: null,
    ...
    actions:{
        ...
        resetTheme(context){
            context.commit('setTheme', deepCopy(defaultThemes['深灰']));
        },
