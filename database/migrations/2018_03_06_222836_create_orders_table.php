...
class CreateOrdersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('orders', function (Blueprint $table) {
            ...
            $table->timestamp('order_time')->default(0)->comment('下单时间');
            $table->decimal('pay_price')->default(0)->comment('支付金额');
            $table->timestamp('pay_time')->default(0)->comment('付款时间');
            $table->decimal('refund_price')->default(0)->comment('退款金额 (聚合父订单为退款总金额)');
            $table->timestamp('refund_time')->default(0)->comment('退款时间 (聚合父订单为最后退款时间)');
