...
class CreateBizerIdentityAuditRecordsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('bizer_identity_audit_records', function (Blueprint $table) {
            $table->increments('id');
            $table->integer('bizer_id')->index()->default(0)->comment('业务员ID');
            $table->string('name')->default('')->comment('身份证姓名');
    ...
    {
        Schema::dropIfExists('bizer_identity_audit_records');
    }
}
