<template>
    ...
    <div>
        ...
        <el-dialog title="编辑小程序配置信息" :visible.sync="isEdit">
            <miniprogram-form
                    :data="scope.row"
                    @cancel="isEdit = false"
                    @save="doEdit"/>
    ...
    export default {
        ...
        methods: {
            ...
            changeStatus(){
                ...
                this.$emit('before-request')
                api.post('/miniprogram/changeStatus', {id: this.scope.row.id, status: status}).then((data) => {
            ...
            del(){
                ...
                this.$confirm(`确定要删除小程序配置 ${data.name} 吗? `, '温馨提示', {type: 'warning'}).then(() => {
                    this.$emit('before-request')
                    api.post('/miniprogram/del', {id: data.id}).then(() => {
                        this.$emit('refresh')
                    }).finally(() => {
                        this.$emit('after-request')
                    })
