...
class CreateGoodsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('goods', function (Blueprint $table) {
            ...
            $table->decimal('purchase_price')->index()->default(0)->comment('采购价[取所有商品规格中最低一个]');
            $table->decimal('origin_price')->index()->default(0)->comment('商品原价[取所有商品规格中最低一个]');
            $table->decimal('discount_price')->index()->default(0)->comment('商品折扣价格 [暂时不用][取所有商品规格中最低一个]');
            ...
            $table->string('spec_name_2')->default('尺码')->comment('商品规格2名称, 默认: 尺码');
            $table->integer('default_spec_id')->default(0)->comment('默认的商品规格ID');
            $table->integer('category_id_1')->index()->default(0)->comment('商品一级分类ID');
            $table->integer('category_id_2')->index()->default(0)->comment('商品二级分类ID');
            $table->integer('category_id_3')->index()->default(0)->comment('商品三级分类ID');
            $table->integer('category_id_4')->index()->default(0)->comment('商品四级分类ID');
